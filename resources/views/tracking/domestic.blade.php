...
@php
    ...
    $milestones = [
        ['label' => 'Order Booked', 'icon' => 'fa-receipt', 'desc' => 'Pickup scheduled in Nepal'],
        ['label' => 'Collected & Sorted', 'icon' => 'fa-box', 'desc' => 'Weighed & tagged at depot'],
        ['label' => 'Highway Express', 'icon' => 'fa-truck-fast', 'desc' => 'Inter-district fleet transport'],
        ['label' => 'Local Dispatch', 'icon' => 'fa-motorcycle', 'desc' => 'Rider assigned to ward'],
...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <section class="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 text-white shadow-xl border border-slate-800">
        <div class="p-6 md:p-8 pb-4">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-xs text-slate-400 flex items-center gap-2">
                        <span><i class="fas fa-truck-fast text-teal-400"></i> Express Fleet Synchronized</span>
                        <span>&middot;</span>
            ...
            <!-- INTER-DISTRICT HIGHWAY ROUTE CORRIDOR -->
                    ...
                    <div class="relative flex items-center justify-between z-10">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span class="text-[11px] font-mono text-slate-400">Bagmati Province Hub</span>
                            </div>
                        </div>
                        ...
                        <div class="hidden md:flex flex-col items-center flex-1 px-8 relative">
                            ...
                            <span class="text-[10px] font-bold tracking-widest uppercase text-slate-400 mt-2 bg-slate-950 px-3 py-0.5 rounded-full border border-slate-800">
                                Prithvi / East-West Express Transit
                            </span>
                        </div>
                        ...
                        <div class="flex items-center gap-3 text-right">
                            <div>
                                <span class="text-[10px] font-bold text-teal-400 uppercase tracking-widest">Destination Ward/City</span>
                                <h4 class="text-base sm:text-lg font-bold text-white leading-snug">
                                    {{ $shipment->receiver_city ?: 'Destination' }}
                                </h4>
                                <span class="text-[11px] font-mono text-slate-400">
                                    @if($shipment->receiver_ward) Ward {{ $shipment->receiver_ward }} &middot; @endif Local Delivery Station
                                </span>
                            </div>
                            <span class="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-teal-500/20 text-teal-400 flex items-center justify-center text-xl shrink-0 border border-teal-500/30">
    ...
    <section class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Weight & Parcel</span>
            <div class="mt-2 flex items-baseline gap-2">
                <span class="text-2xl font-black text-slate-900 font-mono">
                    {{ number_format($shipment->weight ?? 1.0, 2) }}
                </span>
                <span class="text-xs font-bold text-slate-500">KG Net</span>
            </div>
            <p class="text-[11px] text-slate-400 mt-1">Package: {{ ucfirst($shipment->package_type ?? 'Parcel') }}</p>
        </div>

        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Service Class</span>
            ...
            <p class="text-[11px] text-slate-400 mt-1">Guaranteed transit window</p>
        </div>

        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Target Arrival</span>
            <div class="mt-2 flex items-baseline gap-1.5">
                <span class="text-lg font-black text-teal-700">
                    {{ $shipment->estimated_delivery_at ? $shipment->estimated_delivery_at->format('M d · h:i A') : 'Same Day / 24H' }}
                </span>
            </div>
            <p class="text-[11px] text-slate-400 mt-1">Direct to recipient door</p>
        </div>

        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Recipient Pin / Ward</span>
            <div class="mt-2 flex items-center gap-2">
                <span class="h-8 w-8 rounded-xl bg-emerald-50 text-emerald-700 flex items-center justify-center text-sm">
                    <i class="fas fa-map-pin"></i>
                </span>
                <span class="text-xs font-bold text-slate-800">
                    Ward: {{ $shipment->receiver_ward ?? 'Main Hub' }}
                </span>
            </div>
            <p class="text-[11px] text-slate-400 mt-1">Verified Delivery Zone</p>
        </div>
    </section>
            ...
            <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
                <div>
                    <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2">
                        <i class="fas fa-timeline text-teal-600"></i> Domestic Transit Log
                    </h2>
                    <p class="text-xs text-slate-500 mt-0.5">Verified checkpoint scans across regional hubs</p>
                </div>
                <span class="px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-bold">
                    {{ count($events) }} Checkpoints
                </span>
            </div>
            ...
            <section class="bg-white rounded-3xl p-6 border border-slate-200 shadow-2xs">
                ...
                <dl class="divide-y divide-slate-100 text-xs mt-2">
                    <div class="py-3 flex justify-between items-center">
                        <dt class="text-slate-500">Waybill Code</dt>
                    ...
                    <div class="py-3 flex justify-between items-center">
                        ...
                        <dd class="font-medium text-slate-800">{{ $shipment->receiver_city ?: 'Destination' }}</dd>
                    </div>

                    <div class="py-3 flex justify-between items-center">
                        <dt class="text-slate-500">Target ETA</dt>
                        <dd class="font-bold text-teal-700">{{ $shipment->estimated_delivery_at ? $shipment->estimated_delivery_at->format('M d · h:i A') : '24-48 Hours' }}</dd>
                    </div>
                ...
                <div class="mt-4 p-3 rounded-xl bg-slate-50 border border-slate-200/80 text-[11px] text-slate-500 flex items-start gap-2 leading-relaxed">
                    ...
                    <span>Privacy-guaranteed domestic manifest tracking. Personal telephone numbers and full physical addresses are protected from public inspection.</span>
                </div>
            </section>

            <section class="bg-slate-900 rounded-3xl p-6 text-white border border-slate-800">
                <h4 class="font-bold text-sm text-white">Domestic Fleet Support</h4>
                <p class="text-xs text-slate-300 mt-1.5 leading-relaxed">
                    Dispatch inquiries reference: <span class="font-mono text-teal-300 font-bold">{{ $shipment->tracking_number }}</span>.
                </p>
                <div class="mt-4 space-y-2">
                    ...
                    <a href="{{ route('tracking.page') }}" class="w-full py-2.5 px-4 rounded-xl bg-white/10 hover:bg-white/15 text-white font-semibold text-xs flex items-center justify-center gap-2 transition">
                        <i class="fas fa-search"></i> Track Another Delivery
                    </a>
                </div>
            </section>
        </aside>
