...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm">
        <div class="px-6 py-4 border-b">
            <h1 class="text-xl font-semibold text-gray-800">Edit Transit Point</h1>
            <p class="text-sm text-gray-500 mt-1">Update transit point information</p>
        </div>
            ...
            <form method="POST" action="{{ route('overseas.transit-points.update', $transitPoint->id) }}">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="partner_id" required class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500">
                            ...
                            @foreach($partners as $partner)
                                <option value="{{ $partner->id }}" {{ old('partner_id', $transitPoint->partner_id) == $partner->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $transitPoint->name) }}" required 
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($types as $key => $label)
                                <label class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('type', $transitPoint->type) == $key ? 'border-teal-500 bg-teal-50' : '' }}">
                                    <input type="radio" name="type" value="{{ $key }}" {{ old('type', $transitPoint->type) == $key ? 'checked' : '' }} 
                    ...
                    <div>
                        ...
                        <input type="text" name="location" value="{{ old('location', $transitPoint->location) }}" required 
                               class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500">
                        @error('location')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="country" value="{{ old('country', $transitPoint->country) }}" required 
                               class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500">
                        @error('country')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="is_mandatory" value="1" {{ old('is_mandatory', $transitPoint->is_mandatory) ? 'checked' : '' }}
                    ...
                    <div>
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $transitPoint->is_active) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                ...
                <div class="mt-6 flex gap-3 pt-4 border-t">
                    <button type="submit" class="bg-teal-600 text-white px-6 py-2 rounded-lg hover:bg-teal-700 transition">
                        <i class="fas fa-save mr-2"></i> Update Transit Point
                    </button>
