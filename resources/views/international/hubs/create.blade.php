...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">Configure an international gateway hub, transit point, customs modes, and handling operations.</p>
        </div>
    </div>
    ...
    <form action="{{ route('international.hubs.store') }}" method="POST" class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <label class="flex items-center gap-2 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer text-xs">
                            <input type="radio" name="hub_type" value="main_hub" {{ old('hub_type', 'main_hub') === 'main_hub' ? 'checked' : '' }} class="text-indigo-600 focus:ring-indigo-500">
                            <div>
                        ...
                        <label class="flex items-center gap-2 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer text-xs">
                            <input type="radio" name="hub_type" value="transit_point" {{ old('hub_type') === 'transit_point' ? 'checked' : '' }} class="text-indigo-600 focus:ring-indigo-500">
                            <div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="text" name="country" value="{{ old('country') }}" placeholder="e.g. United Arab Emirates, United Kingdom, USA" required class="w-full text-sm px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <input type="text" name="city" value="{{ old('city') }}" placeholder="e.g. Dubai, London, New York" required class="w-full text-sm px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="handling_agency_name" value="{{ old('handling_agency_name') }}" placeholder="e.g. Gulf Express Clearance Services LLC" class="w-full text-sm px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="text" name="agency_contact_person" value="{{ old('agency_contact_person') }}" placeholder="e.g. Operations Manager / Rashid Khan" class="w-full text-sm px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mt-4">
                <div>
                    ...
                    <input type="text" name="agency_phone" value="{{ old('agency_phone') }}" placeholder="e.g. +971-4-1234567" class="w-full text-sm px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-600 dark:text-slate-400 mb-1.5">Dispatch & Pre-alert Email</label>
                    <input type="email" name="agency_email" value="{{ old('agency_email') }}" placeholder="e.g. ops@clearance-partner.com" class="w-full text-sm px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-600 dark:text-slate-400 mb-1.5">Facility / Warehouse Address</label>
                    <input type="text" name="agency_address" value="{{ old('agency_address') }}" placeholder="e.g. Cargo Terminal 2, Air Freight City" class="w-full text-sm px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>
        </div>
        ...
        <div class="pt-2 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <label for="is_active" class="text-sm font-medium text-slate-700 dark:text-slate-300">Hub is active and available for routing and flight manifests</label>
            </div>

            <div class="flex items-center gap-2">
                <label class="text-xs font-bold uppercase text-slate-500">Sort Order:</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="w-20 text-sm px-2.5 py-1.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white">
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-semibold shadow-lg shadow-indigo-600/30 transition">
                <i class="fas fa-save"></i> Save International Hub
            </button>
        </div>
    </form>
</div>
